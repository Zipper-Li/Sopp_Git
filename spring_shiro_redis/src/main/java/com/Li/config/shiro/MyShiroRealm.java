...
public class MyShiroRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
		...
		SimpleAuthenticationInfo authenticationInfo = new SimpleAuthenticationInfo(managerInfo, // 用户
				...
				ByteSource.Util.bytes(managerInfo.getPassSalt()), // salt=username+salt
				getName() // realm name
		);
	...
	protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
		...
		UserInfo managerInfo = (UserInfo) principals.getPrimaryPrincipal();
		// 设置相应角色的权限信息
		for (Role role : managerInfo.getRole()) {
			// 设置角色
			authorizationInfo.addRole(role.getRoleName());
		}
		for (Permission p : managerInfo.getPermission()) {
			// 设置权限
